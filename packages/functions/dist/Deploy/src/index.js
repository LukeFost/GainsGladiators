...
import { keccak256, http, createPublicClient, verifyMessage, createWalletClient, parseGwei } from 'viem';
...
async function sendTransaction(account, to, gweiAmount) {
    let result = {
        derivedPublicKey: account.address,
        to: to,
    ...
    try {
        if (getType == 'sendTx') {
            result = (queries.to && queries.gweiAmount) ?
                await sendTransaction(account, queries.to[0], queries.gweiAmount[0]) :
