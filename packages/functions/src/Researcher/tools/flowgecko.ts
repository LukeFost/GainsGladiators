...
export async function getTrendingPoolsForNetwork(network: string): Promise<string[]> {
    try {
        const response = await axios.get(`https://api.geckoterminal.com/api/v2/networks/${network}/trending_pools`);
        const trendingPools = response.data.data.map((pool: any) => pool.attributes.name);
